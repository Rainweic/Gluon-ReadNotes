...
def _parse_network(network, outputs, inputs, pretrained, ctx, **kwargs):
    ...
    inputs将被转化为Symbol下的Variable，并且用list[]整合起来，输入network中，并且被return
    outputs将被在末尾添加'_output'，并用listp[]整合，然后用整合好的outputs从network中提取输出（Symbol那块的），并返回
    network要么是从model_zoo中加载，要么是已经创建好的，通过.collect_params()收集参数并返回参数（估计gluon与symbol的参数通用）
    ...
    # 将inputs中的str转为symbol中的Variable
    ...
    return inputs, outputs, params  # inputs outputs均是symbol类型的list
